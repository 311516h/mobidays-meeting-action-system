...
- ChatGPT
  - 사용 이유: 과제 요구사항을 빠르게 구조화하고, 문제 재정의와 기획안 흐름을 검토하기 위해 사용했다.
  - 사용 범위: 과제 요구사항 해석, 5페이지 기획안 구성 검토, 기술 의사결정 방향 정리, 제출 전 문서 표현 점검

- Codex
  - 사용 이유: 실제 Python 프로젝트 구현, 파일 수정, 실행 검증, Git 커밋까지 한 흐름으로 보조받기 위해 사용했다.
  - 사용 범위: 프로젝트 스캐폴딩, DuckDB 파이프라인 구현, mock LLM 액션아이템 추출, Streamlit 대시보드, 평가 지표, Slack mock 업데이트 루프, README/AI_USAGE 문서 보강
